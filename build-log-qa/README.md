...
compares the build against a previous one. Its focus is the game's **logging
implementation** — what events the build actually generated and what should
be reported back to the developers.

**How this differs from the other two features:** this pipeline knows nothing
about grading. Whether the logs still *support* the progress-point rubrics is
[grading-readiness-audit/](../grading-readiness-audit/)'s question, and
whether the rubric logic itself computes the right colors is
[rubric-validation/](../rubric-validation/)'s. (Both of those consume this
feature's coverage manifests under `config/coverage/`.)
...
* Weekly: `--baseline build-log-qa/reports/<last-week>`.
