...
The rubric describes the fifth-floor machine solutions inconsistently: the student-action column lists clay/sand combinations per room, the log-tag column lists **sand** for both machines, and the score formula simply awards 1 point per room for a correct first-attempt solution. The current dashboard script (grading-logic/mhs-unit4-point4-grading.md) implements the formula by counting canister changes — exactly one change per layer on the two-layer machine (the game requires both canisters to be set; decision A6, 2026-09-21) and exactly one change on the single-layer machine. The intended correct settings for each canister should be confirmed before this rubric is quoted to teachers.

## Implementation Note (drill task)

The rubric's "layer 3 or layer 4" wording does not match the game as built: in the dialogue database only the fourth-floor choice (`107:5`) reaches the clean-water outcome and ends the task, while the middle choice (`107:4`) yields contaminated water and returns the player to the prompt, exactly like the first-floor, second-floor and bedrock choices. Following the game (decision B3, 2026-09-21, applied to the grading scripts on 2026-09-24), the dashboard counts the middle choice as a wrong attempt: 2 points for the fourth floor on the first attempt, 1 point on the second attempt, 0 otherwise. A student who chooses the middle layer first and the fourth floor second therefore earns 1 point, not 2.
