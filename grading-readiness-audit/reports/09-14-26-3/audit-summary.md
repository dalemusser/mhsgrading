# Gameplay-Log → Progress-Point Grading Audit — build 09-14-26-3

- Logs: `playthrough-logs-and-results/09-14-26-3` — single playthrough, Unit1/Unit2/Unit3/Unit4/Unit5 complete (player `6aa8589346b3c45b562092f8`).
...
4. **Dialogue DB export is 2026-06-10** vs game build 20260914-: text-level conclusions carry that staleness caveat (MEDIUM confidence ceiling).
5. **eventKey↔payload mismatches** exist in 53 DialogueEvent record(s) in this dump; none hit grading keys in this dump, but keyed grading is exposed to them.
...
| U2P1 | Student must complete the map-profile matching independently and with… | 8 event keys | 3/8 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
...
| U2P3 | Windowed rule using client timestamps. Count wrong-direction prompts … | 36 event keys | 6/36 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
...
| U2P5 | Score-based rule. Count positive (correct) and negative (incorrect) a… | 55 event keys | 15/55 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
...
| U3P3 | Score-based rule with a bonus. Count incorrect argument selections, c… | 20 event keys + 1 event type(s) | 10/21 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
| U3P4 | Gate + score-based rule. First, the student must have the gate event … | 11 event keys | 8/11 observed | READY_FOR_GRADING_TEST | EXECUTED_NO_INDEPENDENT_CHECK (yellow) | — | More playtesting: production executed cleanly, but this playthrough gave no independent w… |
...
| U4P2 | This progress point will check how many attempts the player used to f… | 7 event keys + 1 event type(s) | 5/8 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
...
| U4P4 | This progress point is score based. There are two tasks under this pr… | 7 event keys + 1 event type(s) | 7/8 observed | READY_FOR_GRADING_TEST | EXECUTED_NO_INDEPENDENT_CHECK (yellow) | — | More playtesting: production executed cleanly, but this playthrough gave no independent w… |
| U4P5 | This progress point records how the player performs in the flooding a… | 17 event keys | 8/17 observed | READY_FOR_GRADING_TEST | PASS (yellow) | — | None — supported and validated on this playthrough; keep monitoring weekly. |
