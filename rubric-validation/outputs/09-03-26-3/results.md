# Rubric validation results — 09-03-26-3
...
| U2P1 | Escape the Ruin | green | green | PASS |
...
[PASS] U1P3 — Defend the Expedition
       ...
       reason(s) for yellow (expected):
         - WRONG_ARG_SELECTED: attempt_number=3 (yellow nodes hit 2x in window) — student needed multiple tries to build the correct argument
```
...
[PASS] U2P2 — Foraged Forging
       expected=yellow actual=yellow
       reason(s) for yellow (expected):
         - BAD_FEEDBACK: triggering_number=19 (threshold <= 1) — repeated wrong-direction prompts while searching for Toppo
```
...
[PASS] U4P4 — Alien Well Floor 5 + You Know the Drill
       expected=yellow actual=yellow
       reason(s) for yellow (expected):
         - _score: score=0 attempt_time=16 negative_feedback_number=7
         ...
         - BAD_FEEDBACK: negative_feedback_number=7 (> 0) — wrong drilling depths before the clean-water depth
```
...
[PASS] U5P2 — If I Had a Nickel- Floors 3 & 4
       expected=yellow actual=yellow
       reason(s) for yellow (expected):
         - _score: floor3_attempts=24 floor4_attempts=10 score=0
         - TOO_MANY_ATTEMPTS_3: floor3_attempts=24 (>6) — too many condenser/evaporator interactions on the 3rd floor
...
[PASS] U5P3 — What Happened Here?
       expected=yellow actual=yellow
       reason(s) for yellow (expected):
         - WRONG_ARG_SELECTED: negativeCount=11 (>=4) — too many wrong arguments before submitting the correct one
```
